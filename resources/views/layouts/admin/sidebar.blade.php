<div class="be-left-sidebar">
    <div class="left-sidebar-wrapper">
        <a href="#" class="left-sidebar-toggle">Dashboard</a>
        <div class="left-sidebar-spacer">
            <div class="left-sidebar-scroll">
                <div class="left-sidebar-content">
                    <ul class="sidebar-elements">
                        <li class="divider">Menu</li>
                        <li class="#">
                            <a href="#"><i class="icon mdi mdi-home"></i><span>Beranda</span></a>
                        </li>
                        <li class="">
                            <a href="#"><i class="icon mdi mdi-account"></i><span>User</span></a>
                        </li>
                        <li class="">
                            <a href="#"><i class="icon mdi mdi-check-circle"></i><span>Absensi</span></a>
                        </li>
                        <li class="">
                            <a href="#"><i class="icon mdi mdi-tab"></i><span>Keuangan</span></a>
                        </li>
                        <li class="">
                            <a href="#"><i class="icon mdi mdi-ticket-star"></i><span>Ujian</span></a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</div>
